...
class Generator(nn.Module):
    ...
    def forward(self, x, lm, s):  # X is depth image, lm is landmark image, s is source code
        """
        Args:
            x: is depth image extracted from target image
            lm: is landmark image extracted from target image
            s: is latent feature extracted from source image
            masks: is mask, but it's assigned as a land mark image.
...
class Discriminator_img_pix(nn.Module):
    def __init__(self, img_size=256, num_domains=2, self_att=False, max_conv_dim=512):
        ...
        self.output = nn.Sequential(nn.LeakyReLU(0.2), nn.Conv2d(dim_out, dim_out, 4, 1, 0), nn.LeakyReLU(0.2),
...
class Discriminator_img2_pix(nn.Module):
    def __init__(self, img_size=256, num_domains=2, self_att=False, max_conv_dim=512):
        ...
        self.output = nn.Sequential(nn.LeakyReLU(0.2), nn.Conv2d(dim_out, dim_out, 2, 1, 0), nn.LeakyReLU(0.2),
                                    nn.Conv2d(dim_out, 1, 1, 1, 0))
...
def build_model(args):
    # Dependong on which model, output gonna be different
    # pdb.set_trace()
    if args.multi_discriminator:
        ...
        nets = Munch(generator=generator, style_encoder=style_encoder, discriminator=discriminator_img,
    ...
    #                      style_encoder=style_encoder_ema)
        ...
        nets = Munch(generator=generator,discriminator=discriminator_img)
        nets_ema = Munch(generator=generator_ema)
